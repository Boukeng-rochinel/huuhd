...
class OpAcademicYear(models.Model):
    ...
    @api.model
    def _backfill_trimestre_grouping(self):
        """Retrofit les annees academiques creees AVANT l'introduction de la
        hierarchie Trimestre/Sequence (ex: par edutek_cameroun_system quand
        il ne creait encore que 6 Sequences a plat) : si une annee a
        exactement 6 periodes-feuilles sans parent ni enfant, les regroupe
        par 2 sous 3 nouveaux Trimestres. N'invente ni ne modifie aucune
        date existante - cree seulement les 3 conteneurs et relie les
        sequences deja presentes. Idempotent (relance a chaque mise a jour
        de module via un appel <function>, meme idiome que
        op.classe._ensure_default_fee_lines_all_classes) : une annee deja
        regroupee, ou dont la structure ne ressemble pas a 6 sequences a
        plat, est silencieusement ignoree."""
        Term = self.env['op.academic.term']
        for year in self.search([]):
            flat_sequences = year.academic_term_ids.filtered(
                lambda t: not t.parent_term and not t.child_term_ids
            ).sorted('sequence')
            if len(flat_sequences) != 6:
                continue
            for t_index in range(3):
                pair = flat_sequences[t_index * 2: t_index * 2 + 2]
                trimestre = Term.create({
                    'name': 'Trimestre %d' % (t_index + 1),
                    'sequence': 100 + t_index + 1,
                    'academic_year_id': year.id,
                    'term_start_date': pair[0].term_start_date,
                    'term_end_date': pair[-1].term_end_date,
                })
                pair.write({'parent_term': trimestre.id})
