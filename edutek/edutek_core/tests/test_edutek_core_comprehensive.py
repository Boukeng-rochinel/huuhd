...
class TestEdutekCoreComprehensive(TestCoreCommon):

    def setUp(self):
        super(TestEdutekCoreComprehensive, self).setUp()
    ...
    def test_06_security_record_rules(self):
        ...
        faculty_user = self.env['res.users'].create({
            ...
            'group_ids': [(6, 0, [self.env.ref('edutek_core.group_op_faculty').id])]
        })
